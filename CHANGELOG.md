...
### Added
- `logs tail [command] [--lines <n>]` — streams the last N lines (default 50) of a CLI diagnostic log to the terminal in real time, with ANSI color coding for log levels. Useful for monitoring long-running operations such as `setup all` without opening the log file manually.
- `setup permissions status` — shows a summary table of all API permissions currently granted to the agent blueprint and agent identity service principals, including grant type (delegated/application), consent state, and whether the permission is required by the current `a365.config.json`. Exits 1 if any required permission is missing or not consented.
...
### Changed
- `setup all` retry logic now surfaces a `[WRN]` message to the terminal after the third consecutive retry attempt, so users are aware the CLI is waiting on Entra replication rather than hanging silently.
- `logs export` redaction now replaces tenant-specific GUIDs found in resource appIds (not just JWT claims) with consistent `<guid-N>` aliases, improving log correlation across multiple exports from the same environment.
...
### Fixed
- `setup permissions status` no longer throws `NullReferenceException` when the agent identity service principal has no app role assignments. The command now correctly reports all permissions as missing and exits 1.
- `logs tail` on Windows no longer duplicates lines when the log file uses CRLF line endings. Lines are now normalized to LF before display.

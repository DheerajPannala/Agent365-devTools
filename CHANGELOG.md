...
### Fixed
- Azure CLI error detection now uses ARM error codes (e.g. `AuthorizationFailed`, `QuotaExceeded`, `Conflict`) instead of matching English text, so error handling works correctly regardless of Azure CLI language setting (#100)
